...
function bornBeforeFifteen(inventorArr) {
    const result = inventorArr.filter(inventor => inventor.year >= 1500 && inventor.year < 1600)
...
function returnFirstAndLastNames(inventorArr) {
    const result = inventorArr.map(inventor => `${inventor.first} ${inventor.last}`)
    console.log(result)
}
...
const transportation = data.reduce((obj, item) => { // obj represents each value in our array, and item is the index value
    if (!obj[item]) {
